...
**`ContentView.swift`** — Observes `OBDViewModel` via `@StateObject`. Renders four cards: Fuel Trims (STFT + LTFT cells; the header shows the STFT+LTFT total), Fluid Temperatures (Coolant / Trans Fluid / Engine Oil, each with a Normal/Warm/Hot colour badge), Engine (RPM / Load / Injector / Inj Vol — injector pulse width `213C` ms and injection volume `2137` ml; injector replaced the throttle cell, which still rides the standard Mode-01 frame into the tuning CSV), and an optional live TX/RX log card shown only when the `loggingEnabled` setting is on. Includes a `SettingsView` sheet — Listen-Only toggle, polling interval slider (0–10 s, 0.1 s steps), keep-screen-awake (`UIApplication.shared.isIdleTimerDisabled`), and the TX/RX logging toggle — all `@AppStorage`-backed. Uses a `#if DEBUG` extension `init(viewModel:)` + the `OBDViewModel.preview` static factory for SwiftUI previews.
...
passive (pollingDelay wait, default 1.0 s)
  ...
  → queryingInjectionVolume 2137 injection volume: (256A+B)×2.047/65535 → ml (A,B after 61 37; multi-frame)  (Toyota enhanced)
...
Coolant, RPM, throttle, and fuel trims always come from standard Mode-01 PIDs. Engine oil, ATF, injector pulse width, and injection volume have no standard equivalent on this ECU, so they stay Toyota-enhanced (`2151`/`2182`/`213C`/`2137`).

- **Injector pulse width** is mode-21 PID `3C`. The single-frame response `61 3C A B C D E` carries two 16-bit fields; the live pulse width is the **second** one — `(256·C + D) / 1000 → ms`. On-car, `C·D` wobbles with fuel correction (~2.9 ms warm idle, rises under load) while the first field `A·B` is a slow staircase (averaged); Car Scanner shows the `C·D` value in ms. The earlier `21F3` guess returned a `7F` NRC and was replaced by `213C`.
- **Injection volume** is mode-21 PID `37` (multi-frame, declared length `0x11`=17). Volume = `(256·A + B) × 2.047 / 65535 → ml` from the first 16-bit field after `61 37`. Confirmed against Car Scanner (~1.6 ml at idle). (Community lists attribute this volume formula to `213C`, but on this car `213C` is pulse width and `2137` is the volume.)

All five reads (standard + the four enhanced) go through one shared `sendEngineQuery(_:state:status:resetParser:)` helper. It sends `ATCEA` then the engine header (`ATSH7E0`), with 150 ms delays, **only if the header isn't already active** — tracked by the `engineHeaderActive` flag. The header is **established once per session and persists**: the first read sets `7E0` (flag `true`), and the steady-state active loop sends *no AT commands at all* (no per-cycle restore, no re-send), so nothing can change the header — each subsequent cycle is just `pollingDelay` wait + the five OBD commands. The flag is only cleared (forcing a re-establish) at: `runELM327Init` (ends on `ATSH7DF`); a **whole-cycle read failure** in `afterPollCycle` (`if !cycleGotData`, the clone self-heal); and `beginListenWindow` (entering the listen monitor, which toggles CAF/ATMA — there the header *must* be re-set each cycle). There is **no `ATSH7DF` restore between active cycles** — `7DF` is only set once per connect in init, since the standard batch is requested on `7E0`, not the functional header. A 5-second watchdog (`armActiveTimeout`) auto-advances each step.  Entry point: `triggerActiveQuery()` → `beginStandardQuery()`, fired by `scheduleNextActiveQuery()`.
...
The chain advances on the ELM327 **`>` prompt**, not on response-receipt. A parser that finishes a response records the next step in `pendingNext` (`.injector`/`.injectionVolume`/`.engineOil`/`.atf`/`.endCycle`) and returns; `route()` fires it via `dispatch(_:)` when the `>` line arrives (the manager emits `>` as its own sentinel line). Sending the next command *before* `>` — while the adapter is still in its inter-response wait — makes the ELM327 abort it with `STOPPED`, losing the command and stalling ~5 s on the watchdog (observed on-car before this fix). The `>` handler ignores prompts when `pendingNext` is nil (AT-command `OK`s during header re-establish, init). The watchdog is the fallback: it clears `pendingNext` and advances directly if no prompt/response arrives in 5 s. A stray `STOPPED` (or `NO DATA`/`ERROR`) is treated as a fast skip via `isTerminalLine`. The first command of each mode (`beginStandardQuery`, or listen's `beginInjectorPulseQuery`) is sent directly — there's no prior prompt to wait for.
...
beginListenOnly()  ATCM7FF + ATCF7E8   (mask 0x7FF / filter 0x7E8 = exactly 7E8, the engine ECU's response ID)
  ...
  → beginInjectionVolumeQuery 2137 injection volume (active request)
...
`afterPollCycle()` is the branch point: `listenModeActive ? beginListenWindow() : startPassiveMonitoring()`. The poll chain is `Standard → InjectorPulse → InjectionVolume → EngineOil → ATF → afterPollCycle` (the enhanced reads follow the standard frame, injector first; in listen mode the chain starts at `InjectorPulse` since the standard frame is sniffed in the monitor window). Each step's exits (success, NRC, terminal error) set `pendingNext` and advance on the `>` prompt (see Command pacing); the watchdog is the fallback. ATF's exits set `.endCycle` → `afterPollCycle()`, so the mode never falls through to active standard requests. The six standard values are sniffed passively — they update only while another tester (e.g. the Avance48) is polling them on the bus, **and** only outside each brief injector/volume/oil/ATF poll window. The 7E8 filter is also correctness for the monitor phase: `OBDParser` holds one shared multi-frame accumulator that any other CAN ID would reset mid-sequence.
...
"Listen-Only" is a slight misnomer — the poll phase does transmit `213C`/`2137`/`2151`/`2182`. But those enhanced PIDs are not what the Avance48 polls, so this is the minimum transmission needed and still avoids competing for the standard PIDs. (Each enhanced read is an extra transmit per cycle — four now — marginally increasing the footprint on the shared bus.)
...
- **Tuning CSV** (`dataLoggingEnabled`): one row per decoded standard Mode-01 frame — `timestamp,STFT,LTFT,RPM,Throttle,EngineLoad,Coolant,InjectorPulse,InjectionVolume` — written via `logTuningSample()` in both `parseStandardLine` and `parseListeningLine` to `obd_tune_<yyyy-MM-dd_HH-mm-ss>.csv`. The first seven columns are sampled from the same standard frame; `InjectorPulse` (`213C`) and `InjectionVolume` (`2137`) come from separate enhanced reads (polled earlier in the cycle) and so lag the row by about one poll cycle (each logs the most recent read at row time). The Tuning Data Log card shows the row count and a `ShareLink` (AirDrop) to the current session file.
